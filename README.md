...
我做这个工具，是因为真实的使用环境里经常没法舒服地直接使用 Claude Code / Codex：

- iPhone 上挂梯子不方便，临时想让 Agent 帮忙看代码、改 Bug 时，直接从手机打开终端或网页都很别扭。
- 另一台工作电脑因为环境限制不能安装梯子，也就没法稳定访问这些 Agent 服务。
- 但我有一台 Mac 可以正常运行 Claude Code / Codex，并且能访问本地代码仓库。
- 远程让 Agent 改代码时，也需要把可操作范围限定在明确的项目目录里，避免误碰到项目外的文件。

所以与其在每台设备上都折腾环境，不如让 Mac 作为常驻的 Agent 执行机器，再用飞书、企业微信这类 IM 工具作为入口。这样在手机、受限电脑或群聊里发一条消息，就能把任务转给 Mac 上的 Agent 执行，结果再回到 IM 里。
...
通过 `projects` 可以配置多个项目，在聊天中用 `/project` 命令切换。每个项目都对应一个明确的本地目录，Agent 会在当前项目目录下执行任务，从入口层面收窄可操作范围，降低误改项目外文件的风险。

每个飞书机器人还可以通过 `feishuBots[].projects` 绑定自己的项目子集，例如让某个机器人只能访问 `backend`，另一个机器人同时访问 `frontend` 和 `docs`。
